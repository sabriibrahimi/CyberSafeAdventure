...
class CyberSafeGame:
    ...
    def init_level(self):
        ...
        if self.current_level == 5:
            ...
            if hasattr(self.level, 'question_block_positions') and self.level.question_block_positions:
                ...
                chosen = random.sample(platform_candidates,
                                       min(len(platform_candidates), 8)) if platform_candidates else []
                for plat in chosen:
                        ...
                        if self.question_sequence_active:
                            ...
                            if self.question_sequence_index < len(self.question_sequence):
                                ...
                                if correct == 3:
                                    ...
                                    self.player.heal(15)
                                    self.spawn_floating_text("+15 HP", self.player.rect.centerx,
                                                             self.player.rect.top - 20, GREEN)

                                if correct <= 1:

                                    try:
                                        self.player.take_damage(5)
                                        self.spawn_floating_text("-5 HP", self.player.rect.centerx,
                                                                 self.player.rect.top - 20, RED)
                                    except Exception:
                                        pass
                                self.active_question_block = None
                            ...
                            if self.selected_option == self.current_question["correct"]:
                                ...
                                self.spawn_floating_text(f"-{damage_amount} HP", self.player.rect.centerx,
                                                         self.player.rect.top - 20, RED)
                                self.player.invincible = True
                        ...
                        for block in hit_blocks:
                            if block.active:
                                ...
                                if is_below and is_moving_up:
                                    ...
                                    self.question_sequence = random.sample(TRIVIA_QUESTIONS,
                                                                           min(3, len(TRIVIA_QUESTIONS)))
                                    self.question_sequence_index = 0
                                    self.question_sequence_correct = 0
    ...
    def draw(self):
        if self.in_menu:
            ...
            self.ui.draw_password_challenge(self.screen, self.password_input, self.password_feedback,
                                            self.password_strength)
            pygame.display.flip()
            return
        elif self.in_email_sorting:
            if self.current_email_index < len(self.email_list):
                self.ui.draw_email_sorting(self.screen, self.email_list[self.current_email_index],
                                           len(self.email_list) - self.current_email_index, self.email_feedback)
            pygame.display.flip()
            return
        ...
        if not self.game_over:
            ...
            if self.level_intro_active:
                self.ui.draw_level_introduction(self.screen, self.current_level,
                                                self.level.messages[self.current_level])
